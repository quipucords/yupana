...
oc login https://${PROD_OPENSHIFT_HOST}:${PROD_OPENSHIFT_PORT} --token=${PROD_OPENSHIFT_TOKEN}
...
echo "Checking for successful deployment at http://${APP_NAME}-${OPENSHIFT_PROJECT}.${PROD_OPENSHIFT_APP_DOMAIN}${APP_READINESS_PROBE}"
set +e
rc=1
count=0
attempts=100
while [ $rc -ne 0 -a $count -lt $attempts ]; do
  CURL_RES=`curl -I --connect-timeout 2 http://${APP_NAME}-${OPENSHIFT_PROJECT}.${PROD_OPENSHIFT_APP_DOMAIN}${APP_READINESS_PROBE} 2> /dev/null | head -n 1 | cut -d$' ' -f2`
  if [[ $CURL_RES == "200" ]]; then
    rc=0
    echo "Successful test against http://${APP_NAME}-${OPENSHIFT_PROJECT}.${PROD_OPENSHIFT_APP_DOMAIN}${APP_READINESS_PROBE}"
    curl -d "{\"text\": \"${OPENSHIFT_PROJECT}/${APP_NAME} (SUCCESS) - Successful test against http://${APP_NAME}-${OPENSHIFT_PROJECT}.${PROD_OPENSHIFT_APP_DOMAIN}${APP_READINESS_PROBE}.\"}" -H "Content-Type: application/json" -X POST ${SLACK_QPC_BOTS}
    break
  fi
  count=$(($count+1))
  echo "Attempt $count/$attempts"
