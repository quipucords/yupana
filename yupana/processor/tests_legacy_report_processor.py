...
from processor import (legacy_abstract_processor,
                       ...
                       legacy_report_processor,
                       tests_legacy_report_consumer as test_handler)
from prometheus_client import REGISTRY

from api.models import LegacyReport, LegacyReportArchive, LegacyReportSlice, Status


# pylint: disable=too-many-public-methods
# pylint: disable=protected-access,too-many-lines,too-many-instance-attributes
class LegacyReportProcessorTests(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.processor = legacy_report_processor.LegacyReportProcessor()
    ...
    def test_archiving_report(self):
        ...
        self.processor.status = legacy_report_processor.SUCCESS_CONFIRM_STATUS

        self.processor.archive_report_and_slices()
        # assert the report doesn't exist
        with self.assertRaises(LegacyReport.DoesNotExist):
            LegacyReport.objects.get(id=report_to_archive.id)
        # assert the report archive does exist
        archived = LegacyReportArchive.objects.get(account='4321')
        self.assertEqual(json.loads(archived.state_info), [LegacyReport.NEW])
    ...
    def test_archiving_report_not_ready(self):
        ...
        existing_report = LegacyReport.objects.get(id=report_to_archive.id)
        ...
        with self.assertRaises(LegacyReportArchive.DoesNotExist):
            LegacyReportArchive.objects.get(account='4321')
        # assert the processor was reset
        self.check_variables_are_reset()
    ...
    def test_deduplicating_report(self):
        ...
        # assert the report doesn't exist
        with self.assertRaises(LegacyReport.DoesNotExist):
            LegacyReport.objects.get(id=report_to_dedup.id)
        # assert the report archive does exist
        archived = LegacyReportArchive.objects.get(account='4321')
        self.assertEqual(json.loads(archived.state_info), [LegacyReport.NEW])
        # assert the processor was reset
    ...
    def test_determine_retry_limit(self):
        ...
        self.report_record.state = LegacyReport.STARTED
        self.report_record.retry_count = 4
        self.report_record.save()
        self.processor.report_or_slice = self.report_record
        self.processor.determine_retry(LegacyReport.FAILED_DOWNLOAD,
                                       LegacyReport.STARTED)
        self.assertEqual(self.report_record.state, LegacyReport.FAILED_DOWNLOAD)
    ...
    def test_update_report_state(self):
        ...
        self.report_record.save()
        self.processor.next_state = LegacyReport.STARTED
        # set the values we will update with
        self.processor.report_or_slice = self.report_record
        options = {'retry': legacy_abstract_processor.RETRY.increment,
        ...
        self.assertEqual(self.report_record.retry_type, LegacyReport.GIT_COMMIT)
    ...
    def test_assign_report_oldest_time(self):
        ...
        LegacyReport.objects.get(id=older_report.id).delete()

    def test_assign_report_not_old_enough(self):
        """Test the assign report function with young report."""
        # delete the report record
        LegacyReport.objects.get(id=self.report_record.id).delete()
        self.processor.report_or_slice = None
        current_time = datetime.now(pytz.utc)
    ...
    def test_assign_report_oldest_commit(self):
        ...
        self.processor.assign_object()
        self.assertEqual(self.processor.report_or_slice, older_report)
        self.assertEqual(self.processor.report_or_slice.state, LegacyReport.DOWNLOADED)
        # delete the older report object
        LegacyReport.objects.get(id=older_report.id).delete()
    ...
    async def async_test_delegate_state(self):
        ...
        self.report_record.state = LegacyReport.STARTED
        self.report_record.report_platform_id = self.uuid
        self.report_record.upload_ack_status = legacy_report_processor.SUCCESS_CONFIRM_STATUS
        self.report_record.save()
        self.processor.report_or_slice = self.report_record
        ...
        self.report_record.state = LegacyReport.VALIDATED
        self.report_record.save()

        def validation_reported_side_effect():
            """Side effect for async transition method."""
            self.report_record.state = LegacyReport.VALIDATION_REPORTED
            self.report_record.save()
    ...
    async def async_test_delegate_state_exception(self):
        ...
        with patch('processor.legacy_report_processor.LegacyReportProcessor.delegate_state',
    ...
    def test_reinit_variables(self):
        ...
        self.processor.report_platform_id = self.uuid
        self.processor.report_or_slice = self.report_record
        self.processor.state = LegacyReport.NEW
        ...
        self.assertEqual(self.processor.state, LegacyReport.NEW)
        self.assertEqual(self.processor.account_number, '1234')
        self.assertEqual(self.processor.upload_message, self.msg)
        self.assertEqual(self.processor.status, legacy_report_processor.SUCCESS_CONFIRM_STATUS)
    ...
    def test_transition_to_downloaded(self):
        ...
        with requests_mock.mock() as mock_req:
            ...
            self.assertEqual(self.report_record.state, LegacyReport.DOWNLOADED)
    ...
    def test_transition_to_downloaded_exception_fail(self):
        ...
        def download_side_effect():
            ...
            raise legacy_report_processor.FailDownloadException()

        with patch('processor.legacy_report_processor.LegacyReportProcessor._download_report',
                   side_effect=download_side_effect):
            self.processor.transition_to_downloaded()
            self.assertEqual(self.report_record.state, LegacyReport.FAILED_DOWNLOAD)
    ...
    def test_transition_to_validated(self):
        ...
        self.processor.report_or_slice = self.report_record
        self.report_slice.state_info = json.dumps([LegacyReportSlice.PENDING])
    ...
    def test_transition_to_validated_report_exception(self):
        ...
        self.assertEqual(self.report_record.upload_ack_status,
                         legacy_report_processor.FAILURE_CONFIRM_STATUS)
    ...
    def test_transition_to_validated_general_exception(self):
        ...
        with patch('processor.legacy_report_processor.LegacyReportProcessor._validate_report_details',
            ...
            self.processor.transition_to_validated()
            self.assertEqual(self.report_record.state, LegacyReport.VALIDATED)
            self.assertEqual(self.processor.status, legacy_report_processor.FAILURE_CONFIRM_STATUS)
    ...
    async def async_test_transition_to_validation_reported(self):
        ...
        self.assertEqual(self.processor.report.state, LegacyReport.VALIDATION_REPORTED)
    ...
    async def async_transition_to_validation_reported_failure_status(self):
        ...
        self.processor.state = LegacyReport.VALIDATED
        self.processor.status = legacy_report_processor.FAILURE_CONFIRM_STATUS
        self.processor.upload_message = {'request_id': self.uuid}
        self.processor._send_confirmation = CoroutineMock()
        await self.processor.transition_to_validation_reported()
        with self.assertRaises(LegacyReport.DoesNotExist):
            LegacyReport.objects.get(id=report_to_archive.id)
        archived = LegacyReportArchive.objects.get(account='43214')
        self.assertEqual(archived.state,
                         LegacyReport.VALIDATION_REPORTED)
    ...
    def test_update_slice_exception(self):
        ...
        self.assertEqual(self.report_slice.state, LegacyReportSlice.PENDING)
    ...
    def test_extract_and_create_slices_mismatch(self):
        ...
        self.processor.account_number = '0001'
        buffer_content = test_handler.create_tar_buffer(report_files)
        with self.assertRaises(legacy_report_processor.FailExtractException):
    ...
    def test__extract_and_create_slices_failure_invalid_json(self):
        ...
        buffer_content = test_handler.create_tar_buffer(report_files)
        with self.assertRaises(legacy_report_processor.RetryExtractException):
            self.processor._extract_and_create_slices(buffer_content)
    ...
    def test_download_response_content_bad_url(self):
        ...
        with requests_mock.mock() as mock_req:
            mock_req.get(self.payload_url, exc=requests.exceptions.HTTPError)
            with self.assertRaises(legacy_report_processor.RetryDownloadException):
    ...
    def test_download_response_content_missing_url(self):
        ...
        with self.assertRaises(legacy_report_processor.FailDownloadException):
            self.processor.upload_message = {}
            self.processor._download_report()
    ...
    def test__extract_and_create_slices_general_except(self):
        ...
        with patch('processor.legacy_report_processor.tarfile.open',
    ...
    def test_calculating_queued_reports(self):
        ...
        retry_commit_report = LegacyReport(
            upload_srv_kafka_msg=json.dumps(self.msg),
            account='4321',
            report_platform_id=self.uuid2,
            state=LegacyReport.DOWNLOADED,
        ...
        same_commit_report = LegacyReport(
            upload_srv_kafka_msg=json.dumps(self.msg),
            account='4321',
            report_platform_id=self.uuid2,
            state=LegacyReport.DOWNLOADED,
        ...
        # delete the older report object
        LegacyReport.objects.get(id=older_report.id).delete()
        LegacyReport.objects.get(id=retry_commit_report.id).delete()
        LegacyReport.objects.get(id=too_young_report.id).delete()
        LegacyReport.objects.get(id=same_commit_report.id).delete()

    def test_state_to_metric(self):
        """Test the state_to_metric function."""
        self.processor.state = LegacyReport.FAILED_DOWNLOAD
